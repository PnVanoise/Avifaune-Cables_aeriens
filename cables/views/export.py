...
from cables.views import year_extract_p, year_extract_t, years_p, years_t, \
        R_HIG, R_SEC, to_int, add_header_row, flatten
...
def zs_to_dict(item):
    poteaux = (
        ...
        (item.nb_poteaux_inventories_risque_fort or 0) +
        (item.nb_poteaux_inventories_risque_secondaire or 0),
        item.nb_poteaux_equipes_risque_fort,
        item.nb_poteaux_equipes_risque_secondaire,
        (item.nb_poteaux_equipes_risque_fort or 0) +
        (item.nb_poteaux_equipes_risque_secondaire or 0))
    qfilter = TInventairePoteauxErdf.id_zone_sensible == item.id_zone_sensible
    poteaux_year = tuple(
            get_nb_poteaux(item, year, qfilter) for year in years_p)
    troncons = (
        item.m_troncons_inventories_risque_fort,
        item.m_troncons_inventories_risque_secondaire,
        (item.m_troncons_inventories_risque_fort or 0) +
        (item.m_troncons_inventories_risque_secondaire or 0),
        item.m_troncons_equipes_risque_fort,
        item.m_troncons_equipes_risque_secondaire,
        (item.m_troncons_equipes_risque_fort or 0) +
...
def get_communes(dept, life=None):
    ...
    communes_poteaux = map(
        lambda x: x.insee, DBSession.query(TCommune.insee)
        .join(TInventairePoteauxErdf)
        .join(TEquipementsPoteauxErdf)
        .distinct())
    communes_troncons = map(
        lambda x: x.insee, DBSession.query(TCommune.insee)
...
def commune_to_dict(item):
    ...
    poteaux = (item.nom_commune, hig, sec, hig +
               sec, hig_eq, sec_eq, hig_eq + sec_eq)
    poteaux_year = {
            year: get_nb_poteaux(item, year, pfilter) for year in years_p}
    ...
    troncons = (t_hig, t_sec, t_hig + t_sec, t_hig_eq,
                t_sec_eq, t_hig_eq + t_sec_eq)
    troncons_year = {
